  ...
  componentDidMount () {
    ...
    this.backHandler = BackHandler.addEventListener('hardwareBackPress', () => {
      ...
      if (currentRouteName === 'HomeScreen' || currentRouteName === 'ScreenHome' || currentRouteName === 'ScreenLogin') {
        ...
        // console.log('nav.routes[0].routes.length=>', nav.routes[0].routes.length)
        // console.log('nav.routes[0].routes[0].isDrawerOpen=>', nav.routes[0].routes[0].isDrawerOpen)

        if (nav.routes.length > 0 && nav.routes[0].routes.length > 0 && !nav.routes[0].routes[0].isDrawerOpen) {
          // console.log('ouuuuuttttttt')
