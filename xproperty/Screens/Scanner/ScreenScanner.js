...
export default class ScreenScanner extends React.Component {
  state = {
    flash: 'off',
    zoom: 0,
    autoFocus: 'on',
  ...
  takePicture = async function () {
    if (this.camera) {
      ...
      const callback = path(['navigation', 'state', 'params', 'callback'], this.props)
  ...
  renderTextBlocks = () => (
    <View style={styles.facesContainer} pointerEvents='none'>
      {this.state.textBlocks.map(r => {
        console.log('textBlock ===> ', r)
        return this.renderTextBlock(r)
      })}
    </View>
  );
